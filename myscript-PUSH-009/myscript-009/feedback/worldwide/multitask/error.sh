...
hdfsDir=/user/lestat/stats/$hiveDatabase/multitask/error/$thedate
keyPrefix='multitask.error'
...
hive -e \
"use $hiveDatabase; \
INSERT OVERWRITE Directory '$hdfsDir' select concat('$keyPrefix', '.', feedbackId, '_', stepId, '.', eventName) as thekey, thedate, count(distinct pid) as value, '$inserttime' as lmt from $hiveTable where thedate='$thedate' and sid like 'rsys%' and success=false and feedbackId like 'rinter2%' and stepId!='\000'  and length(eventName) > 1 group by feedbackId, stepId, eventName, thedate;"
